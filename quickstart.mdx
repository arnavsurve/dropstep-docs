...
You will need access to an OpenAI API key. You can get one from [OpenAI](https://openai.com/api/).
...
Navigate to your `my-dropstep-intro` directory in your terminal and run:
...
The key part is the agent's summary message and the final JSON output, which by default contains a `summary` field with the agent's response to your prompt. This demonstrates the browser agent's ability to perform tasks in a web browser and return structured data.

<Info>
You can also find this output JSON in `.dropstep/output/visit_dropstep_site_output.json`. Visit the [Structured Output Guide](/structured-output) to learn more about customizing an agent's structured output format.
</Info>
